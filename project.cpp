...
// 6. Глобальні об'єкти в статичній пам'яті 
Car car_pool[5] = { // 7. Масив об'єктів
	Car("Toyota", "Camry", 2020, 50.0, "AA1234AA", 10.0, 5000, false),
	Car("Honda", "Civic", 2018, 40.0, "BB5678BB", 8.0, 7000, false),
	Car("BMW", "X5", 2022, 120.0, "CC9012CC", 25.0, 2000, false),
	Car("Tesla", "Model 3", 2023, 100.0, "DD3456DD", 20.0, 1000, false),
	Car("Ford", "Focus", 2017, 35.0, "EE7890EE", 7.0, 15000, true)
};

Client client_registry[2] = { // 7. Масив об'єктів
	Client("Ivanenko I.I.", "i.ivan@mail.com", "AB123456", "0987654321", 500.0, false, 0, Car()),
	Client("Petrov P.P.", "p.petrov@mail.com", "CD654321", "0971234567", 100.0, false, 0, Car())
};

// 6. Об'єкти у динамічній пам'яті 
Client* dynamic_client = new Client("Olenko O.O.", "o.olen@mail.com", "EF987654", "0501112233", 800.0, false, 0, Car());

// 6, 7. Динамічний масив об'єктів 
Car* dynamic_car_array = new Car[3]{
	Car("Fiat", "Punto", 2016, 30.0, "OP1212OP", 5.0, 20000, false),
	Car("Lexus", "RX", 2021, 150.0, "QR3434QR", 30.0, 3000, false),
	Car("Skoda", "Octavia", 2019, 60.0, "ST5656ST", 12.0, 10000, false)
};
...
// 7. ДЕМОНСТРАЦІЯ КОНСТРУКТОРІВ ТА СТАТИЧНИХ ФУНКЦІЙ

void constructorsAndStaticsDemo() {
	sectionSeparator("1. CONSTRUCTORS, STATIC & REFERENCE DEMO (P. 1, 2, 3, 4, 5, 7)");

	// 3. Демонстрація статичного поля і методу 
	Car::displayTotalCarCount();
	Client::displayTotalClientCount();

	// 7. Демонстрація конструкторів Car
	cout << "\n--- Car Constructor Demo ---" << endl;
	Car carA; // 1.1. Конструктор без параметрів (П. 1, П. 7)
	Car carB("Audi", "A4", 2021, 90.0, "BBBB", 15.0, 1000, true); // 1.2. Конструктор з параметрами (П. 1, П. 7)
	Car carC = carB; // 1.3. Конструктор копіювання (П. 1, П. 7)
	Car carD("Nissan", "Leaf", 60); // 2. Конструктор зі списком ініціалізації (Константа) (П. 2, П. 7)
	carD.displayCarInfo();
	Car::displayTotalCarCount(); // 3. Демонстрація роботи статичного поля

	// 7. Демонстрація конструкторів Client
	cout << "\n--- Client Constructor Demo ---" << endl;
	Client clientA; // 1.1. Конструктор без параметрів (П. 1, П. 7)
	Client clientB("Voloshyn V.", "v.volosh@mail.com", "EF445566", "0661230000", 250.0, false, 0, carA); // 1.2. Конструктор з параметрами (П. 1, П. 7)
	Client clientC = clientB; // 1.3. Конструктор копіювання (П. 1, П. 7)
	string externalID = "GLOBAL_REF_777";
	Client clientD("External Client", "ZZ999999", externalID); // 2. Конструктор зі списком ініціалізації (Посилання) (П. 2, П. 7)
	clientD.displayClientInfo();
	Client::displayTotalClientCount(); // 3. Демонстрація роботи статичного поля

	// 4. Демонстрація зовнішніх функцій (П. 4)
	sectionSeparator("4. EXTERNAL FUNCTION DEMO");
	calculateCarProfitEstimate(carB); // 4. Виклик функції поза класом
	calculateClientNetWorth(clientB); // 4. Виклик функції поза класом

	// 5. Демонстрація функцій, що повертають об'єкт класу (П. 5)
	sectionSeparator("5. FUNCTION RETURNS OBJECT DEMO");
	Car returnedCarReport = createDamageReport(carB); // 5. Виклик функції, що повертає об'єкт Car
	returnedCarReport.displayCarInfo();

	Client returnedNewClient = createNewClient("Demo Client", 50.0); // 5. Виклик функції, що повертає об'єкт Client
	returnedNewClient.displayClientInfo();
}



void carMethodsDemo() {
	sectionSeparator("2. CAR METHODS DEMONSTRATION (P. 9, 5, 12)");
	int carIndex;
	Car& selectedCar = selectCar(carIndex);

	cout << "\nSelected Car: " << selectedCar.plateNumber << endl;
	...
	Car damagePlaceholder("Placeholder", "Damage", 0, 0.0, "DUMMY", 0.0, 0, false);
	Car damageReport = selectedCar.returnCar(selectedCar.mileage + 100, damagePlaceholder); 
	if (damageReport.getMake() != "N/A") {
		cout << "[M5] Car returned. Damage reported: " << damageReport.getModel() << endl;
	}
	...
	selectedCar.allocateAndSortMileage(); 
}


void clientMethodsDemo() {
	sectionSeparator("3. CLIENT METHODS DEMONSTRATION (P. 9, 5, 12)");
	int clientIndex;
	...
	selectedClient.allocateAndSortDuration(); 
}
...
void dynamicMemoryDemo() {
	sectionSeparator("5. DYNAMIC MEMORY AND POINTER DEMONSTRATION (P. 6, 7, 11)");
...
void showMainMenu() {
	...
	cout << "1. Constructors, Static & External Function Demo (P. 1, 2, 3, 4, 5, 7)" << endl;
	cout << "2. Car Methods Demonstration (Interactive Selection) (P. 5, 9, 12)" << endl;
	cout << "3. Client Methods Demonstration (Interactive Selection) (P. 9, 12)" << endl;
	cout << "4. SCENARIO: Client Rents a Car (Interactive Interaction) (P. 10)" << endl;
	cout << "5. Dynamic Memory and Pointer Demonstration (P. 6, 7, 11)" << endl;
...
int main() {
	...
	// 3. Фінальна демонстрація статичних полів перед виходом
	Car::displayTotalCarCount();
	Client::displayTotalClientCount();

	// 6. Очищення динамічної пам'яті
